...
export const updateTask = async (taskId, userId, taskData, workspaceId) => {
    ...
    const actions = [];
    if (titleChanged) {
        actions.push(`Renamed task from ${oldTask.title} to ${updatedTask.title}`);
    }
    if (statusChanged) {
        actions.push(`Changed ${updatedTask.title} status from ${oldTask.status} to ${updatedTask.status}`);
    }
    if (priorityChanged) {
        actions.push(`Changed ${updatedTask.title} priority from ${oldTask.priority} to ${updatedTask.priority}`);
    }
    // assigning task  
    if (!oldTask.assignedToId && updatedTask.assignedToId) {
        actions.push(`Assigned ${updatedTask.title}`);
    }
    else if (oldTask.assignedToId && !updatedTask.assignedToId) {
        actions.push(`Unassigned ${updatedTask.title}`);
    }
    else if (assigneeChanged) {
        actions.push(`Reassigned ${updatedTask.title}`);
    }
    // ------------
    if (dueDateChanged) {
        actions.push(`Updated due date for ${updatedTask.title}`);
    }
    // changin content 
