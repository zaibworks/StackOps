...
const Login = () => {
  ...
  <div className="absolute bottom-[-10rem] right-[-10rem] h-[25rem] w-[25rem] rounded-full bg-blue-500/20 blur-[100px]" />
      ...
      <div className='mb-8 text-center'>
          <h1 className='text-xl font-semibold tracking-tight text-zinc-100'>Stack<span className='text-orange-400'>Ops</span></h1>
          <h2 className="mt-4 text-3xl font-semibold tracking-tight text-zinc-100"> Welcome back</h2>
          <p className="mt-2 text-sm text-zinc-400"> Organize work, track progress, and stay in sync.</p>
      </div>
   <form onSubmit={handleSubmit} className='space-y-4'>
    ...
    <div>
      ...
      <input className='w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 text-zinc-100 outline-none transition-all
      placeholder:text-zinc-500 focus:border-orange-400 focus:ring-1 focus:ring-orange-400'
      type="password" placeholder='Secure password' value={password} onChange={(e)=>setPassword(e.target.value)} />
    </div>
    {
  error && (
...
<button className=' w-full rounded-xl bg-orange-500  px-4 py-3 font-medium text-zinc-950 transition-all duration-200 hover:bg-orange-400 disabled:cursor-not-allowed
disabled-opacity-50
' type='submit' disabled={loading}>{loading ? 'Signing you in...' : 'Sign in'}</button> 
   </form>
   <p className='mt-6 text-center text-sm text-zinc-400 gap-y-2'>
    Don&apos;t have an account?{""}
    <span onClick={()=>navigate('/signup')} className='cursor-pointer font-medium text-orange-400 transition-colors hover:text-orange-300'>
       Create account
    </span>
    </p>
      </div>
